...
const Editpopup: React.FC<EditPopupProps> = ({ onClose, data }: any) => {
  ...
  const handleCheckboxChange = (
    ...
    if (event.target.checked) {
      ...
      setSelectedGroups((prevSelectedGroups) =>
        prevSelectedGroups?.filter((g) => g._id !== group._id)
      );
    }
  };

  const handleRemoveGroups = () => {
    const updatedGroups = removedData?.group?.filter(
      (g: any) =>
        !selectedGroups?.some((selectedGroup) => selectedGroup._id === g._id)
    );
    setRemovedGroups((prevRemovedGroups) => [
      ...prevRemovedGroups,
      ...selectedGroups,
    ]);
    const updatedData = {
  ...
  return (
    <section className="flex h-[90vh] max-w-[800px]  flex-col w-[50%] items-start gap-[44px] bg-white p-4 rounded-md overflow-auto">
      ...
      <div className="px-[40px] rounded-md border w-full py-2">
        ...
        <ul className="grid grid-cols-2 border px-5 max-h-[15rem] overflow-y-auto gap-5 mt-5">
          {removedData &&
            removedData?.group?.length > 0 &&
            removedData?.group.map((group: any, index: any) => (
              <li key={index} className="flex items-center justify-between">
                <span className="text-md text-gray-500 py-1 px-5  font-semibold capitalize">
                  {group.name}
                </span>
                <input
                  type="checkbox"
        ...
        <div className="flex justify-end items-end  py-2">
          <button
            className=" border-2 rounded-md border-[#005D7F]  text-[#005D7F]  cursor-pointer px-[10px]  "
        ...
        <ul className="grid grid-cols-2 gap-2 mt-5  max-h-[10rem] overflow-y-auto">
          {groupsData &&
            ...
            groupsData
              ?.filter(
                (group: any) =>
                  !removedData?.group?.some((g: any) => g?._id === group?.id)
              )
              ?.map((group: any, index: any) => (
                <li
                  key={index}
                  className="flex items-center gap-5 list-decimal"
                >
                  <label
                    htmlFor={`group-${index}`}
      ...
      <div
        className="bg-white cursor-pointer absolute right-[8%] top-[1%] p-3 rounded-full shadow-md"
        onClick={onClose}
      >
        <img src={close} alt="" />
      </div>
